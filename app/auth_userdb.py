...
class driver():
    ...
    def __init__(self, userdb):
        ...
        st = os.stat(userdb)
        # when running inside container we dont need to worry about
        # groups permissions, so removing this part
        if False:
    ...
    def login(self, username, password, **kwargs):
        ...
        try:
            ...
            if e.errno == errno.ENOENT:
                logging.info(f'unkown user {username}')
                raise
            raise
        ...
        if len(datal) != 5:
            logging.info(f'db corrupted? {username}')
            raise
        ...
        if hashed_password_input != hashed_password:
            logging.info(f'bad passwd {username}')
            raise
